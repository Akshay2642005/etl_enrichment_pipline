...
def _detect_boilerplate(
    ...
    try:
        ...
        logger.warning(
            "Boilerplate detection LLM call failed for %s '%s'",
            item_type,
            item_name,
        )
    ...
    **without** making any live database queries.
    ...
    def _score_documentation(
        ...
        for table in tables:
            ...
            if desc:
                ...
                if (
                    len(desc) < 80
                    and boilerplate_checks_remaining > 0
                ):
        ...
        ) if table_desc_lengths else 0.0
        ...
        er_with_meaning = sum(
            1 for er in entity_rels
            if bool(er.get("business_meaning", "").strip())
        )
        er_coverage = (
            er_with_meaning / max(len(entity_rels), 1)
            if entity_rels
            else 1.0
        )
    ...
    def _generate_recommendations(
        ...
        try:
            ...
            if isinstance(result, _RecommendationsResult):
    ...
    def _fallback_recommendations(
        ...
        if not recs:
            recs.append(
                "Schema quality is good. Continue monitoring for regressions."
            )
        return recs
