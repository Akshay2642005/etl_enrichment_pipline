...
import { Target, Lightbulb, TrendingUp, Rocket, Loader2, RefreshCw, AlertCircle, Database, Server, PlugZap } from 'lucide-react';
import { regenerateCategoryInsights, fetchEmbeddingStatus } from '../services/api';
...
// Module-level constants — stable references, safe to use in useCallback/useEffect deps.
...
export const InsightsView = () => {
  ...
  // Stable callback — uses getState() for a non-reactive snapshot of current
  // insights so this function never needs to be recreated when data changes.
  const handleGenerate = useCallback(async () => {
    if (!currentConnectionId) {
      setError('No active connection. Please extract a database first.');
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      // getState() gives the latest store value without adding data as a dep.
      let current: Record<string, any> = useAppStore.getState().insightsData ?? {};

      // Generate each domain sequentially so the UI updates progressively.
      // regenerateCategoryInsights computes a SHA-256 delta against the
      // existing stored insights and only writes to the DB when something
      // actually changed — so re-runs are cheap if results are identical.
      for (const cat of CATEGORIES) {
        try {
          const result = await regenerateCategoryInsights(currentConnectionId, cat);
  ...
  }, [currentConnectionId, setIsLoading, setError, setData]);

  // Auto-generate once when a connection is active but insights haven't been
  // loaded yet.  The ref prevents re-firing for the same connection ID even
  // when the component re-renders (e.g. during progressive data updates).
  const autoGeneratedForRef = useRef<string | null>(null);

  useEffect(() => {
    const hasInsights = !!data && Object.keys(data).length > 0;
    if (
      currentConnectionId &&
      !hasInsights &&
      !isLoading &&
      !error &&
      autoGeneratedForRef.current !== currentConnectionId
    ) {
      autoGeneratedForRef.current = currentConnectionId;
      handleGenerate();
    }
  }, [currentConnectionId, data, isLoading, error, handleGenerate]);
  ...
  // No active connection — user must extract a database first
  if (!currentConnectionId && !hasAnyData && !isLoading && !error) {
    return (
      <div className="flex flex-col items-center justify-center h-full max-w-2xl mx-auto text-center px-4">
        <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center mb-6">
          <PlugZap className="w-10 h-10 text-slate-400 dark:text-slate-500" />
        </div>
        <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-4">
          No Connection Active
        </h2>
        <p className="text-slate-600 dark:text-slate-400 mb-8 text-lg">
          Insights are generated from a saved database connection. Extract and save a database first,
          then return here to generate KPIs, insights, and opportunities.
        </p>
      </div>
    );
  }
  ...
  if (!hasAnyData && !isLoading && !error) {
    return (
      <div className="flex flex-col items-center justify-center h-full max-w-2xl mx-auto text-center px-4">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-8 text-lg">
          Generate specialized KPIs, insights, and opportunities tailored specifically for Operations,
          Finance, and other domains from your connected database.
        </p>
        {embeddingStatus.status === 'failed' && (
          <div className="mb-6 w-full max-w-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 text-sm text-amber-700 dark:text-amber-400">
            ...
            Background embedding encountered an error. Insights can still be generated from the enriched schema directly.
          </div>
        )}
        <button
      ...
      <div className="flex space-x-2 overflow-x-auto pb-2 border-b border-slate-200 dark:border-slate-800 hide-scrollbar mb-4">
        {MAIN_TABS.map(tab => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 whitespace-nowrap font-semibold text-sm rounded-t-xl transition-all ${activeMainTab === tab
            ...
            {CATEGORIES.map(cat => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 whitespace-nowrap text-sm rounded-full transition-all border ${activeCategory === cat
